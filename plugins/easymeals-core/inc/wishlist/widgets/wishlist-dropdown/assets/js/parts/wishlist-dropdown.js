(function ($) {
				...
				$.ajax({
					...
					success: function (response) {
						if (response.status === 'success') {
							...
							$(document).trigger('easymeals_core_wishlist_item_is_removed', [removeItemID]);
						} else {
	...
	$(document).on('easymeals_core_wishlist_item_is_added', function (e, addedItemID, addedUserID) {
		var $holder = $('.qodef-wishlist-dropdown');
    ...
    $(document).on('easymeals_core_wishlist_item_is_removed_outside', function (e, removedItemID) {
        var $holder = $('.qodef-wishlist-dropdown');

        if ($holder.length) {
            $holder.each(function () {
                var $thisHolder = $(this),
                    $link = $thisHolder.find('.qodef-e-remove[data-id=' + removedItemID + ']');

                $link.closest('.qodef-m-item').fadeOut(200).remove();
            });
        }
    });
