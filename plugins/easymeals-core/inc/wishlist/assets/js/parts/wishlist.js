(function ($) {
				...
				$wishlistLink.each(function () {
					...
					$thisWishlistLink.off().on('click', function (e) {
						...
						if (qodef.body.hasClass('logged-in')) {
								...
								$.ajax({
									...
									success: function (response) {
										...
										if (response.status === 'success') {
											...
											$(document).trigger('easymeals_core_wishlist_item_is_added', [itemID, response.data.user_id]);
										} else {
											$responseMessage.html(response.message).addClass('qodef--show').fadeIn(200);
										}
							...
							} else if (itemID !== 'undefined' && $thisWishlistLink.hasClass('qodef--added')) {
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        ...
                                        if (response.status === 'success') {
                                            ...
                                            $(document).trigger('easymeals_core_wishlist_item_is_removed_outside', [itemID]);
                                        } else {
                                            $responseMessage.html(response.message).addClass('qodef--show').fadeIn(200);
                                        }
							...
							// Trigger event.
                            $(document.body).trigger('easymeals_membership_trigger_login_modal');
						}
					});
				});
			}
	...
	$(document).on('easymeals_core_wishlist_item_is_removed easymeals_core_wishlist_item_is_removed_outside', function (e, removedItemID) {
		var $wishlistLink = $('.qodef-wishlist .qodef-m-link');
