(function ($) {
	...
	$(document).on('easymeals_trigger_get_new_posts', function () {
		qodefSwiper.init();
		qodefMagnificPopup.init();
	});
	...
	var qodefSwiper = {
		...
		getOptions: function ($holder) {
			var sliderOptions = typeof $holder.data('options') !== 'undefined' ? $holder.data('options') : {},
				...
				effect = sliderOptions.effect !== undefined && sliderOptions.effect !== '' ? sliderOptions.effect : 'default',
			...
			if ( $holder.hasClass('qodef-recipe-list') && $holder.hasClass('qodef-col-num--1') ) {
				effect = 'fade';
			}
			...
			return {
				on: {
					init: function () {
						...
						qodef.body.trigger('easymeals_trigger_swiper_is_initialized', [$holder]);
					}
				}
			};
		}
...
(function ($) {
	...
	$(document).on('easymeals_trigger_get_new_posts', function (e, holder) {
	...
	var qodefFilter = {
		...
		setVisibility: function (holder, item, triggerEvent) {
			...
			if (!isShowAllFilter && !listHasVisibleItems && qodefFilter.hasLoadMore(holder)) {
				qodef.body.trigger('easymeals_trigger_load_more', [holder]);
			} else {
...
(function ($) {
	...
	var qodefMobileHeader = {
		...
		initDropDownMobileMenu: function () {
			var $dropdownOpener = $('#qodef-page-mobile-header nav .menu-item-has-children > a');
...
(function ($) {
	...
	$(document).on('easymeals_trigger_load_more', function (e, $holder, nextPage) {
				...
				success: function (response) {
					...
					if (response.status === 'success') {
                        ...
                        $itemsHolder.waitForImages(function () {
                            ...
                            setTimeout(function () {
                                qodef.body.trigger('easymeals_trigger_get_new_posts', [$holder, response.data, nextPage]);
